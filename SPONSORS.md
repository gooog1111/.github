<div align="center">

[English version](#support-the-projects)

# Поддержать проекты

</div>

Спасибо, если мои проекты оказались полезны. Поддержка помогает тратить больше
времени на доработки, документацию, тесты на реальном железе и поддержку
репозиториев в рабочем состоянии.

## Платформы

<p align="left">
  <a href="https://boosty.to/gooog1111/" target="_blank">
    <img src="https://img.shields.io/badge/Boosty-gooog1111-f15f2c?style=for-the-badge" alt="Boosty">
  </a>
  <a href="https://www.donationalerts.com/r/gooog1111" target="_blank">
    <img src="https://img.shields.io/badge/DonationAlerts-gooog1111-f57c00?style=for-the-badge" alt="DonationAlerts">
  </a>
</p>

## Быстрый перевод по России

<p align="left">
  <a href="https://t.tb.ru/c2c-qr-choose-bank?requisiteNumber=+79996363556&bankCode=100000000004" target="_blank">
    <img src="https://img.shields.io/badge/СБП-Т Банк | Sber-blue?style=for-the-badge&logo=none" alt="СБП">
  </a>
</p>

## Криптовалюта и токены

Перед переводом обязательно проверьте сеть. Переводы в другой сети могут быть
потеряны без возможности восстановления.

| Активы | Сеть | Адрес |
| --- | --- | --- |
| USDT, USDC, GRAM, AAPLx, GOOGLx, METAx | TON | `UQA73kPkNHudFD5yV7DuP-GuXO1ExTpqH0gNioQX8sY4fU6L` |
| BTC | Bitcoin | `bc1q6ny6adsgp0s5ngsng4adclqq9d6hyvqw625g8f` |
| ETH | Ethereum | `0x0D52F429b284205DAb11c0B94af386d5e9899229` |
| TRX | Tron | `TSioV1pZMwAuTVxJnrht7BQrn5CrTmvS8g` |
| SOL | Solana | `GPKWRiZi1iyvbWJZ9YjpdJqesvE3cNrTKJqRmeg1y9KH` |

## Спасибо

Даже звезда репозиторию, issue с подробным описанием проблемы или pull request
с исправлением тоже очень помогают проектам.

---

<div align="center">

[Русская версия](#поддержать-проекты)

# Support the projects

</div>

Thank you if my projects have been useful to you. Support helps me spend more
time on improvements, documentation, testing with real hardware, and keeping the
repositories in good working shape.

## Platforms

<p align="left">
  <a href="https://boosty.to/gooog1111/" target="_blank">
    <img src="https://img.shields.io/badge/Boosty-gooog1111-f15f2c?style=for-the-badge" alt="Boosty">
  </a>
  <a href="https://www.donationalerts.com/r/gooog1111" target="_blank">
    <img src="https://img.shields.io/badge/DonationAlerts-gooog1111-f57c00?style=for-the-badge" alt="DonationAlerts">
  </a>

</p>

## Card2Card

<a href="https://multitransfer.online/ru/card-to-card?account=41001291239620&client=yoomoney" target="_blank">
    <img src="https://img.shields.io/badge/VISA-purple?style=for-the-badge&logo=none" alt="VISA">
  </a>

## Crypto and tokens

Please check the network before sending funds. Transfers sent through the wrong
network may be lost permanently.

| Assets | Network | Address |
| --- | --- | --- |
| USDT, USDC, GRAM, AAPLx, GOOGLx, METAx | TON | `UQA73kPkNHudFD5yV7DuP-GuXO1ExTpqH0gNioQX8sY4fU6L` |
| BTC | Bitcoin | `bc1q6ny6adsgp0s5ngsng4adclqq9d6hyvqw625g8f` |
| ETH | Ethereum | `0x0D52F429b284205DAb11c0B94af386d5e9899229` |
| TRX | Tron | `TSioV1pZMwAuTVxJnrht7BQrn5CrTmvS8g` |
| SOL | Solana | `GPKWRiZi1iyvbWJZ9YjpdJqesvE3cNrTKJqRmeg1y9KH` |

## Thank you

Even starring a repository, opening a detailed issue, or sending a pull request
helps the projects a lot.
